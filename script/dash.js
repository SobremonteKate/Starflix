...
var onLogins = function() {
    $("#frmLogins").submit(function(event) {
        event.preventDefault();
        $('#logins').block(formatBlock());
        var loginObj = $("#frmLogins").serializeArray();


        $.post('php/dashboard.php', loginObj, function(data) {
            if (data.isSuccess) {
                $("admin-msgs").html(alert);
                $('#lname').val('');
                $('#fname').val('');
                var msg = data.msg;
